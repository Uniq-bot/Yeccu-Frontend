...
const CategorySelector = ({ value, onChange, disabled = false, label = "Select Category", type = 'blog' }) => {
    ...
    }, [type, getAllCategory, initializeBlogCategories])

    const categories = type === 'blog' ? blogCategories : allCategories
    const isLoading = type === 'blog' ? isCategoryLoading : isLoadingCategories
